...
export class ModelloDao {
   ...
   */
  async findByCreatorId(creatorId: number): Promise<Modello[]> {
  ...
  async findAllPaginated(
    ...
    return await Modello.findAndCountAll({
      attributes: ['id', 'nome', 'dimensioni_y', 'dimensioni_x', 'createdAt', 'creatore_id'],
      order: [['createdAt', 'DESC']],
      limit,
      offset
    });
   ...
   */
  async getModelInfo(modelId: number, transaction?: Transaction): Promise<{
    model: Modello | null;
    ...
    return {
      model,
      isCreator: (userId: number) => model.creatore_id === userId,
      dimensions: { x: model.dimensioni_x, y: model.dimensioni_y },
      grid: model.griglia
    };
  }
  ...
  /**
   * Ottiene le dimensioni di un modello
   */
  async getDimensions(modelId: number, transaction?: Transaction): Promise<{ x: number; y: number } | null> {
    const model = await this.findById(modelId, transaction);
    return model ? { x: model.dimensioni_x, y: model.dimensioni_y } : null;
  }
  ...
  /**
   * Verifica se le coordinate sono valide per un modello
   */
  async areCoordinatesValid(modelId: number, x: number, y: number, transaction?: Transaction): Promise<boolean> {
    const dimensions = await this.getDimensions(modelId, transaction);
   ...
   */
  async getCellValue(modelId: number, x: number, y: number, transaction?: Transaction): Promise<number | null> {
    const grid = await this.getGrid(modelId, transaction);
    if (!grid || !await this.areCoordinatesValid(modelId, x, y, transaction)) {
      return null;
    }
