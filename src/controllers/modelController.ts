...
import { isValidCoordinate, normalizeCoordinates } from '../utils/coordinateValidator';
...
export const createModel = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const newBalance = await utenteDao.deductTokensAndGetBalance(userId, costoCreazione);
  ...
  try {
    ...
    if (!startPos || !goalPos) {
      throw ErrorFactory.createError(ErrorTypes.BadRequest, 
        'Coordinate di partenza e destinazione sono obbligatorie. Formati supportati: {startX, startY, goalX, goalY} o {start: {x, y}, end: {x, y}}');
    }

    // Validazione coordinate usando utility centralizzata
    if (!isValidCoordinate(startPos, modello.dimensioni_x, modello.dimensioni_y)) {
      throw ErrorFactory.createError(ErrorTypes.BadRequest, 'Coordinate di partenza fuori dai limiti della griglia');
    }

    if (!isValidCoordinate(goalPos, modello.dimensioni_x, modello.dimensioni_y)) {
      throw ErrorFactory.createError(ErrorTypes.BadRequest, 'Coordinate di destinazione fuori dai limiti della griglia');
    }
