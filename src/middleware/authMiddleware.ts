...
export const authenticateToken = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    req.user = {
      id: user.id,
      email: user.email,
      token_rimanenti: user.token_rimanenti,
      ruolo: user.ruolo
    };
...
export const calculateModelCreationCost = async (req: AuthenticatedRequest): Promise<number> => {
  ...
  if (!griglia || !Array.isArray(griglia)) {
    throw new Error('Griglia non valida per calcolo costo');
  }
  const totaleCelle = griglia.length * (griglia[0]?.length || 0);
  return totaleCelle * 0.05;
};

export const calculateUpdateCost = async (req: AuthenticatedRequest): Promise<number> => {
  const { celle } = req.body;
  if (!celle || !Array.isArray(celle)) {
    throw new Error('Celle non valide per calcolo costo');
  }
  return celle.length * 0.35;
};
