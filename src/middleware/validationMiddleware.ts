 ...
 * @param fields - Array of field names that should be arrays
 */
export const validateArrayFields = (fields: string[]) => {
    return (req: Request, res: Response, next: NextFunction): void => {
        for (const field of fields) {
            if (req.body[field] && !Array.isArray(req.body[field])) {
                return next(ErrorFactory.createError(ErrorTypes.BadRequest, `Il campo ${field} deve essere un array`));
            }
        }
